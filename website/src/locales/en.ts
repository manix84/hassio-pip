...
export const en = {
    ...
    currentStatus: {
      body: [
        ...
        "Post-1.0 compatibility and release-readiness work is underway. The Home Assistant integration can guide camera setup with ordered next steps, calibrate a camera against a receiver, preview and save recommended defaults, use manual TV-safe restream URLs, expose go2rtc and Frigate helper metadata for manual restream setup, flag when a restreamed source is likely needed, expose receiver service health, show suggested next steps on the receiver device, and report whether the installed receiver is current, degraded, legacy, or incompatible. GitHub Releases now include APK, HACS zip, and Play Console-ready AAB assets while Play Store account verification remains the external blocker.",
      ],
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          "Android TV devices can reject unsupported codecs or very high-resolution streams. HA TV PiP supports snapshot fallbacks, separate stream camera entities, manual restream URLs, automatic MJPEG fallback, MJPEG-first mode, receiver capability checks, and camera compatibility testing. If HLS/MJPEG are unavailable or only snapshots work, calibration results flag that a TV-safe restreamed source is likely needed and include manual go2rtc and Frigate helper metadata. Start with a lower-resolution or MJPEG camera entity, or save a tested go2rtc/Frigate HLS or MJPEG URL as a per-camera default. Guided provider setup, WebRTC, and transcoding are planned future provider paths.",
      },
      {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "HLS, MJPEG, snapshot, and manual restream URL paths with calibration, visible compatibility results, saved recommendations, per-camera defaults, and documented future provider hooks.",
        status: "complete",
      },
      {
    ...
    roadmap: {
      ...
      items: [
        ...
        "WebRTC support",
        "Remote mode",
        "HACS distribution and Play Store preparation",
      ],
