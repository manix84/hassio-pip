...
4. Runs `Android AAB Release 📦` to build and stage the release Android App Bundle for future Play Console upload.
...
7. Runs `Publish Release 🚀` to generate release notes from `WHATSNEW.md`, create draft GitHub Release `vX.Y.Z` with the Android APKs, Android App Bundle, versioned integration zip, and stable HACS integration zip already attached, then publishes it.
8. Runs `Cleanup 🧹` as a final visible workflow stage.
