...
Stage 7 is complete in `0.24.0`. It adds `stream_type: auto`, `stream_type: hls`, `stream_type: mjpeg`, and `stream_type: snapshot` to `ha_tv_pip.show_camera`. Automatic mode prefers HLS, falls back to snapshot when stream resolution fails, and keeps the snapshot preview visible if receiver playback fails after an HLS URL is accepted. MJPEG is explicit opt-in while compatibility is tested across more camera integrations.
