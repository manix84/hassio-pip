...
The GitHub release workflow can upload the signed AAB to Play Console when Play upload is explicitly enabled:

```txt
GOOGLE_PLAY_SERVICE_ACCOUNT_JSON
```

`GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` is the full JSON service account key for a Play Console user with access to `com.hatvpip.receiver`. The upload job is disabled unless the repository variable `PLAY_STORE_UPLOAD_ENABLED` is set to `true`.
...
5. Runs `HA Integration Release 🏠` to package the Home Assistant integration zips.
6. Runs `Release Asset Check 🔎` to validate APK names, APK/AAB archive shape, integration zip names, HACS zip layout, manual zip layout, icon presence, ignored paths, and manifest version consistency.
7. Runs `Play Store AAB Upload 📲` to upload the AAB to Play Console when `PLAY_STORE_UPLOAD_ENABLED=true`; otherwise it records a successful skip.
8. Runs `Publish Release 🚀` to generate release notes from `WHATSNEW.md`, create draft GitHub Release `vX.Y.Z` with the Android APKs, Android App Bundle, versioned integration zip, and stable HACS integration zip already attached, then publishes it.
...
ha-tv-pip-android-release-v1.2.3.aab
...
HACS also expects repository brand assets. The repo keeps presentation assets in root `brand/` for HACS, root `icon.png` / `logo.png` compatibility aliases for older/simple presentation paths, and installed integration assets in `custom_components/ha_tv_pip/brand/` for Home Assistant 2026.3+ local brand image support. The integration root also includes matching `icon.png` / `logo.png` aliases so both stable HACS zips and manual-install zips carry a simple icon fallback.
...
- Android release App Bundle asset: `ha-tv-pip-android-release-vX.Y.Z.aab`.
...
`ha_tv_pip.setup_camera` is the preferred guided workflow for users. Without `restream_url`, it runs camera calibration and returns `setup_mode: calibration` plus `setup_summary`, `summary`, and `action_plan`. With `restream_url`, it runs restream validation and returns `setup_mode: restream_source`; set `save: true` to save a valid source in the same action.

`ha_tv_pip.calibrate_camera` wraps the compatibility test in a friendlier lower-level workflow. It returns a `summary` object with compatibility, recommended stream type, recommendation reason, whether defaults were saved, and a next-step hint. The response also includes `action_plan`, which gives support tools and dashboards a stable primary action key, readable label, suggested HA TV PiP service, safe payload, optional fields to try, and short notes. Use `save: true` to store the recommended stream strategy plus any explicit calibration fields as per-camera defaults.
...
Use `ha_tv_pip.suggest_restream_source` to generate advisory manual restream setup values for a selected camera and receiver. It returns candidate stream names, go2rtc/Frigate-style HLS/MJPEG URL patterns, provider help, the effective `restream_base_url`, `setup_summary`, ordered `setup_steps`, a safe `test_restream_source` payload for the first HLS candidate, a safe MJPEG fallback test payload, and a safe follow-up `save_restream_source` payload. Helper payloads include both the legacy `service` key and a copyable Home Assistant `action` key. It does not create provider streams.

Use `ha_tv_pip.test_restream_source` to validate a candidate manual restream URL before saving it. It infers HLS or MJPEG from the URL, checks that the URL looks like a playable stream endpoint rather than a provider base URL, checks receiver stream capability metadata, optionally checks reachability from Home Assistant, and returns a `save_action` payload with `service`, `action`, `target`, and `data` when the URL should be saved as a per-camera default. Set `save: true` to persist the candidate in the same action after validation passes.
